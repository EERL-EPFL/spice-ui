...
export function displayCoordinatesToLogicalWell(
  ...
  if (
    ...
    console.error(
      `Invalid logical coordinates: (${logicalRow},${logicalCol}) for grid ${qtyCols}x${qtyRows}`,
    );
    return "ERR";
  }
...
export function transformCellToWellCoordinate(
  ...
  if (
    ...
    console.error(
      `Invalid original coordinates: (${originalRow},${originalCol}) for grid ${qtyCols}x${qtyRows}`,
    );
    return "ERR";
  }
