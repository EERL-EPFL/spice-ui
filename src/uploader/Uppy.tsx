...
export const UppyUploader = ({
    ...
    new Uppy({
      ...
      onBeforeFileAdded: (currentFile, files) => {
        ...
        if (currentFile.size > 50 * 1024 * 1024) {
          notify(`File ${currentFile.name} is too large (max 50MB)`, {
            type: "error",
          });
          return false;
        }
  ...
  useEffect(() => {
    const handleUploadStart = () => {
      ...
      if (queuedFiles > 1000) {
        notify(
          `📦 Starting bulk upload of ${queuedFiles} files. This may take several minutes...`,
          {
            type: "info",
          },
        );
    ...
    const handleError = (error: any) => {
      ...
      setUploadDetails({
        successful: [],
        failed: [{ name: "Unknown file", error: errorMessage }],
      });
    ...
    const handleUploadError = (file: any, error: any, response: any) => {
      ...
      if (error?.isRequestError && error?.source?.status === 409) {
        ...
        error?.message &&
        error.message.includes("already exists in experiment")
      ) {
        errorMessage = error.message;
        isFileConflict = true;
      } else if (response?.status === 409) {
      ...
      if (isFileConflict) {
        notify(
          `⚠️ ${errorMessage}. Delete the existing file first if you want to re-upload.`,
          { type: "warning" },
        );
      }
      ...
      <Box sx={{ flex: compact ? "0 0 auto" : 1 }}>
        ...
        <DragDrop
          ...
          note={
            compact
              ? instructionText
              : `${instructionText} • Multiple files supported • Max 50MB per file`
          }
        />
