    ...
    new Uppy({
        ...
        if (files.length > 0) {
          ...
          try {
            ...
            if (result.errors.length > 0) {
              ...
              notify(
                `Parsing warnings: ${result.errors.length} issues found (see console)`,
                { type: "warning" },
              );
            }
            ...
            notify(
              `Processed ${result.rows.length} phase change records from merged.xlsx`,
              { type: "success" },
            );
          } catch (error) {
            notify(`Error processing merged.xlsx file: ${error.message}`, {
              type: "error",
            });
      ...
      onAfterResponse: (response) => {
        ...
        if (response.status === 200) {
          notify(
            `Phase change data uploaded successfully. Processed ${parsedResponse.processed_count} records.`,
            { type: "success" },
          );
          refresh();
        } else {
          notify(
            `Error uploading phase change data: ${parsedResponse.detail?.message || parsedResponse.detail}`,
            { type: "error" },
          );
        }
      },
