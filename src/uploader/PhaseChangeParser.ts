...
export class PhaseChangeParser {
  ...
  static parseWorksheet(worksheet: XLSX.WorkSheet): ExcelPhaseChangeData {
    ...
    try {
      ...
      for (let i = 0; i < Math.min(5, jsonData.length); i++) {
        ...
        const trayNameCells = row.filter(
          (cell) =>
            cell === "P1" || cell === "P2" || cell === "P3" || cell === "P4",
        );
        const wellCoordCells = row.filter(
          (cell) => typeof cell === "string" && /^[A-Z]\d+$/.test(cell.trim()),
        );
      }
  ...
  static parseTemperatureLoggerFormat(jsonData: any[][]): ExcelPhaseChangeData {
    ...
    if (headerRowIndex === -1) {
      return {
        rows: [],
        errors: ["Could not find header row with Date and Time columns"],
      };
    }
  ...
  static parseComplexPhaseChangeFormat(
    jsonData: any[][],
  ): ExcelPhaseChangeData {
    const rows: PhaseChangeRow[] = [];
    const errors: string[] = [];
    ...
    try {
      ...
      for (let i = 0; i < Math.min(20, jsonData.length); i++) {
        ...
        const trayNameCount = row.filter(
          (cell) =>
            cell === "P1" || cell === "P2" || cell === "P3" || cell === "P4",
        ).length;
        if (trayNameCount >= 50 && trayRowIndex === -1) {
          // Need many tray names
          trayRowIndex = i;
        }

        // Look for well coordinates (A1, A2, B1, etc.) - need multiple coordinates
        // And make sure it's NOT the same row as tray names
        const wellCoordCount = row.filter(
          (cell) => typeof cell === "string" && /^[A-Z]\d+$/.test(cell.trim()),
        ).length;
        if (wellCoordCount >= 50 && wellRowIndex === -1 && i !== trayRowIndex) {
          // Different from tray row
          wellRowIndex = i;
        }
      ...
      if (trayRowIndex === -1 || wellRowIndex === -1 || headerRowIndex === -1) {
        return {
          rows: [],
          errors: [
            "Could not find required header rows (tray names, well coordinates, or Date/Time header)",
          ],
        };
      }
  ...
  private static buildColumnMap(
    ...
    for (let i = 0; i < headerRow.length; i++) {
      ...
      if (header.includes("date") || header.includes("time")) {
        ...
        const probeNumber = probeMatch
          ? parseInt(probeMatch[1] || probeMatch[2])
          : null;
  ...
  private static parseDataRow(
    ...
    const phaseData = new Map<
      string,
      { trayName: string; wellName: string; phase: number }
    >();
    ...
    phaseData.forEach(({ trayName, wellName, phase }) => {
      ...
      for (let probeNum = 1; probeNum <= 8; probeNum++) {
        const tempCol = columnMap.find(
          (col) => col.type === "temp" && col.probeNumber === probeNum,
        );
  ...
  private static parseDateTime(rawDateTime: any): string {
    ...
    if (!isNaN(numValue) && numValue > 40000) {
      // Excel dates after 2009
      // Excel date serial number
      const excelEpoch = new Date(1900, 0, 1);
      date = new Date(
        excelEpoch.getTime() + (numValue - 2) * 24 * 60 * 60 * 1000,
      );
    } else {
  ...
  private static parseWellCoordinate(
    wellName: string,
