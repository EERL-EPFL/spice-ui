 ...
 */
const TrayConfigurationRegionInput: React.FC<{ trayConfigurationId?: any }> =
  React.memo(({ trayConfigurationId }) => {
...
const TrayConfigurationSelector: React.FC<{
  ...
  fallbackTrayId?: any;
  isLoading?: boolean;
}> = ({ defaultTrayId, fallbackTrayId, isLoading }) => {
  if (isLoading) {
...
const CreateComponent: React.FC = () => {
  ...
  const {
    ...
    error: traysError,
  } = useGetList("tray_configurations", defaultTraysParams);
  ...
  const trayToUse = useMemo(
    () => defaultTrayId || fallbackTrayId,
    [defaultTrayId, fallbackTrayId],
  );
    ...
    <Create redirect="show" transform={transformData}>
      ...
      >
        {!traysLoading && !allTraysLoading && allTrays?.length === 0 && (
          ...
          >
            ❌ No tray configurations found. Please create a tray configuration
            first.
          </div>
        )}
        {!traysLoading && !defaultTrayId && allTrays?.length > 0 && (
          ...
          >
            ⚠️ No default tray configuration found. Using "{allTrays[0]?.name}"
            as fallback. Consider setting a tray configuration as default.
          </div>
        )}
        <TextField source="id" />
