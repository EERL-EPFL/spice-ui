...
export const TimePointVisualization = () => {
  ...
  if (!record?.tray_configuration_id) {
    return (
      <Alert severity="warning">
        No tray configuration assigned to this experiment. Assign a tray
        configuration to visualize time points.
      </Alert>
    );
  }
  ...
  if (!results || !results.summary || results.summary.total_time_points === 0) {
    return (
      <Alert severity="info">
        No time point data available. Upload a merged.xlsx file to see phase
  ...
  const wellsFrozen = allWells.filter(
    (w: any) => w.final_state === "frozen",
  ).length;
  const wellsLiquid = allWells.filter(
    (w: any) => w.final_state === "liquid",
  ).length;
      ...
      <Grid container spacing={2}>
                ...
                <Box mt={3} p={2} bgcolor="grey.50" borderRadius={1}>
                  ...
                  {selectedWell.first_phase_change_seconds !== null && (
                    <Typography variant="body2">
                      ...
                      {formatSeconds(selectedWell.first_phase_change_seconds)}
                    </Typography>
                  )}
                  {(selectedWell.sample_name || selectedWell.sample?.name) && (
                    <Typography variant="body2">
                      <strong>Sample:</strong>{" "}
                      {selectedWell.sample?.name || selectedWell.sample_name}
                    </Typography>
                  )}
                  {(selectedWell.treatment_name ||
                    selectedWell.treatment?.name) && (
                    <Typography variant="body2">
                  ...
                  {selectedWell.treatment?.sample?.name && (
                    <Typography variant="body2">
                      <strong>Treatment Sample:</strong>{" "}
                      {selectedWell.treatment.sample.name}
                    </Typography>
                  )}
              ...
              <Box mt={3}>
                ...
                <Box display="flex" alignItems="center" gap={1} mt={0.5}>
                  <Box
                    ...
                    border="1px solid #ccc"
                  />
                  <Typography variant="caption">No freeze / No data</Typography>
                </Box>
              </Box>
            </CardContent>
          </Card>
        ...
        <Grid item xs={12} md={9}>
          <Box sx={{ maxHeight: "80vh", overflowY: "auto", overflowX: "auto" }}>
            {trays.length === 0 ? (
              <Alert severity="warning">
                No tray configurations found. Please check the tray
                configuration setup.
              </Alert>
            ) : (
                    ...
                    <Box key={resultTray.tray_id} sx={{ flexShrink: 0 }}>
                      ...
                      <TrayWellGrid
                        tray={{
                          ...
                          name:
                            resultTray.tray_name ||
                            `Tray ${resultTray.tray_id}`,
                          order_sequence: 1,
                          trays: [
                            {
                              id: resultTray.tray_id,
